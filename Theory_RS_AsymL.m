function [OP] = Theory_RS_AsymL(KK,LL,LdB,yS,xPU,yPU,xPB,yPB,rho,alpha,beta,eta,PdB,IdB,Rth)
% hop by hop RS
Ip          = 10.^(IdB/10);
PP          = 10.^(PdB/10);
% KK          = length(NumRelay);
...
for kk = 1 : KK
    ...
    Sum1Sub3 = 0;
    for ii = 0 : LL-1
             Sum1Sub3 = Sum1Sub3 + fvartheta(LL,ii)/2.*...
                 fchi(LD_Dk,gmTH,OMG_Ik(ii,rho,LD_Ik(kk)),Ip);
    end
    Sum1 = Sum1 + 1 - Sum1Sub3;
end
OP = ones(1,length(PP)).*(Sum1);
